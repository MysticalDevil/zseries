...
- [Use the TUI Dashboard](#use-the-tui-dashboard)
...
The dashboard is read-only and split into two sections:

- `TOTP`: card-style entries with source, code, and countdown
- `Readonly`: imported non-TOTP entries that are stored but not code-generating
