...
pub fn saveVault(allocator: std.mem.Allocator, io: std.Io, data_dir: []const u8, password: []const u8, payload: model.VaultPayload, params: SecurityParams) ![]u8 {
    ...
    const dir_path = std.fs.path.dirname(path) orelse return error.InvalidPath;
    // Stage the new vault bytes in a temporary file under the same directory,
    // then atomically rename it into the real vault path.
    var tmp = try ztmpfile.tempfileIn(allocator, dir_path);
    defer tmp.deinit();
    var write_buffer: [4096]u8 = undefined;
    var writer = tmp.file().writer(io, &write_buffer);
    try writer.interface.writeAll(bytes);
    try writer.interface.flush();
    const persisted = try tmp.persist(path);
