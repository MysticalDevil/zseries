...
Quick local smoke test without preparing your own vault or fixtures:
...
| `tui` | Open a searchable dashboard | `ztotp tui` |
...
`ztotp tui` opens a lightweight searchable terminal dashboard.

It shows:

- a TOTP card grid with current codes and countdowns
- a separate readonly section for imported non-TOTP entries
- a live search query filtered by issuer, account, source, and kind
...
Run a quick end-to-end local smoke flow with built-in test entries:
